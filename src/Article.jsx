...
export default function Article({ slug, navigateTo, backLabel }) {
  ...
  if (!article) {
    return (
      <div className="max-w-3xl mx-auto w-full px-4 mb-24 text-center py-20">
        ...
        <button
          onClick={() => navigateTo('column', '/column')}
      ...
      <article className="prose prose-slate max-w-none space-y-12 bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-slate-100">
        {article.sections.map((section, idx) => (
          <section key={idx} className="space-y-4">
            <h2 className="text-2xl font-black text-slate-900 border-l-4 pl-4" style={{ borderColor: PRIMARY_COLOR }}>
              {section.heading}
            </h2>
            {section.body.split('\n\n').map((para, i) => (
