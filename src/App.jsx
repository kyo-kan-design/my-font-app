...
body {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-4 md:p-8">
      <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
            ...
            JP Font Pairing Simulator
          </h1>
          <p className="text-slate-500 text-sm mt-1">日本語フォントの最適な組み合わせをデザインする</p>
        </div>
        ...
        <div className="flex bg-white p-1 rounded-xl shadow-sm border border-slate-200 w-fit">
          {[
            ...
            >
              ...
              <span className="hidden sm:inline">{VIEWPORT_SIZES[id].label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-4 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <section>
                ...
                <textarea 
                  ...
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all resize-none"
                />
              </section>
              <div className="pt-2">
                <div className="grid grid-cols-2 gap-2">
                  {PRESET_TEXTS.map((t, idx) => (
                    <button
                      ...
                      className={`py-2 text-xs rounded-lg border transition-all ${activePreset === t.title ? 'bg-blue-50 border-blue-200 text-blue-700 font-bold' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}
                    >
                      {t.title}
                    </button>
                  ))}
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-slate-50 space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">ウェイト (太さ)</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] text-slate-400 block mb-1">見出し: {headingWeight}</label>
                    <input type="range" min="300" max="900" step="100" value={headingWeight} onChange={(e) => setHeadingWeight(parseInt(e.target.value))} className="w-full accent-blue-600" />
                  </div>
                  <div>
                    <label className="text-[10px] text-slate-400 block mb-1">本文: {bodyWeight}</label>
                    <input type="range" min="300" max="900" step="100" value={bodyWeight} onChange={(e) => setBodyWeight(parseInt(e.target.value))} className="w-full accent-blue-600" />
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-50 space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">カラー</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[10px] font-mono text-blue-600">{headingSize}px</span>
                    </div>
                    <input type="range" min="24" max="80" value={headingSize} onChange={(e) => setHeadingSize(e.target.value)} className="w-full accent-blue-600" />
                  </div>
                  
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Body Size</span>
                      <span className="text-[10px] font-mono text-blue-600">{bodySize}px</span>
                    </div>
                    <input type="range" min="12" max="24" value={bodySize} onChange={(e) => setBodySize(e.target.value)} className="w-full accent-blue-600" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="range" min="1.2" max="2.4" step="0.1" value={lineHeight} onChange={(e) => setLineHeight(e.target.value)} className="w-full accent-blue-600" />
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-1">
                      ...
                      <input type="range" min="-0.05" max="0.3" step="0.01" value={letterSpacing} onChange={(e) => setLetterSpacing(parseFloat(e.target.value))} className="w-full accent-blue-600" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3 pt-8 mt-6 border-t border-slate-100">
              <button 
                onClick={randomize}
                className="flex items-center justify-center gap-2 w-full py-3 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl font-bold transition-all active:scale-95 border border-slate-200"
              >
                <RefreshCcw className="w-4 h-4" /> シャッフル
              </button>
              <button 
                onClick={copyCSS}
                className="flex items-center justify-center gap-2 w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-black transition-all shadow-lg shadow-blue-200 active:scale-95"
              >
        ...
        <div className="lg:col-span-8 flex justify-center">
          ...
          >
            ...
            <div className="p-8 md:p-16 min-h-[600px] flex flex-col justify-center relative bg-white transition-all">
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-[0.02] pointer-events-none">
                <Type className="w-full h-full max-w-md" />
              </div>
            </div>
          </div>
        </div>
      ...
      {showToast && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-300 z-50">
          <CheckCircle2 className="w-5 h-5 text-green-400" /> 
          <span className="text-sm font-bold">CSSをコピーしました</span>
        </div>
      )}
    </div>
  );
